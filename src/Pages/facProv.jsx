...
import FormFactura from './formFacturaProv'; // Modal para crear/vincular facturas
import { ENDPOINTS } from '../api';
import './personal.css'; // Mantiene la consistencia visual del diseño corporativo
...
export default function Facturas() {
    ...
    // Asegúrate de definir ENDPOINTS.facturas o un endpoint equivalente en tu archivo api.js
    ...
    const normalizeFacturasData = (rawData) => {
        ...
        const payload = Array.isArray(rawData)
            ? rawData
            : (rawData.results || rawData.data || rawData.facturas || rawData.factura_set || rawData.invoice_set || rawData.proveedores || []);
        ...
        const nestedKeys = ['facturas', 'invoice_set', 'factura_set', 'facturas_proveedor', 'factura'];
    ...
    const loadFacturas = async () => {
        try {
            ...
            console.error(err);
            setError("No se pudo conectar con el servidor de facturación de proveedores.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('¿Eliminar esta factura de forma permanente? Esto no borrará las entradas asociadas.')) return;
        try {
            const token = localStorage.getItem('token');
    ...
    // Desglosador tolerante para el objeto foráneo 'proveedor'
    const getNombreProveedor = (proveedor) => {
        if (!proveedor) return 'Proveedor no asignado';
        return proveedor.nombre || String(proveedor);
    };

    // Cuenta cuántos movimientos (entradas) están vinculados mediante el ManyToMany
    ...
    const formatDate = (dateStr) => {
        if (!dateStr) return '—';
        // Formato básico legible si viene en YYYY-MM-DD
    ...
    const filteredFacturas = useMemo(() => {
        ...
        return facturas.filter(item => {
            ...
            const nomProveedor = String(item.nombre_proveedor || item.proveedor?.nombre || item.proveedor || '').toLowerCase();
            ...
            <main className="pro-main">
                <header className="pro-top-nav">
                    <div className="header-left">
                        ...
                        <h1>Control de Cuentas - Proveedores SNZ</h1>
                    </div>
                </header>
                        ...
                        <div className="section-header">
                            <div>
                                ...
                                <p style={{ fontSize: '0.85rem', color: '#64748b', marginTop: '4px' }}>
                                    Control de saldos e historial de entradas liquidadas
                                </p>
                            </div>
                                ...
                                <table className="personal-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>No. Factura</th>
                                            <th>Proveedor</th>
                                            <th style={{ textAlign: "center" }}>Entradas Vinculadas</th>
                                        ...
                                        {loading ? (
                                            ...
                                            filteredFacturas.map(item => (
                                                <tr key={item.id}>
                                                    ...
                                                    {/* CAMBIO: En lugar de usar la función pasándole item.proveedor, lee el campo del serializer */}
                                                    <td>
                                                        <span className="user-name" style={{ fontWeight: '500' }}>
                                                            {item.nombre_proveedor || item.proveedor?.nombre || getNombreProveedor(item.proveedor) || 'Sin Proveedor asignado'}
                                                        </span>
                                                    </td>
                                                    <td style={{ textAlign: "center" }}>
                                                        <span className="dept-tag" style={{ background: '#eff6ff', color: '#1d4ed8', fontWeight: '600' }}>
                                                            {getCantidadMovimientos(item.movimiento)} remisiones
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td style={{ textAlign: "right", color: '#047857', fontWeight: '700', fontSize: '1rem' }}>
